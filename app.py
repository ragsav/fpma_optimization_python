...
def portfolio_var(W):
    ...
    std = get_std()
    # print("std = ", std)
    covariance_mat = get_covariance_mat(get_X())
    # print("covar = ", covariance_mat)
    corr_df = np.array(get_correlation_mat(covariance_mat))
    # print("corr = ", corr_df)
    std_w = np.array(get_weighted_std(W))
...
#         print("-->before mutation",self.w," and old f = ",self.f)
        while(a == b):
            ...
            w[b] = dna
        self.w = w
        self.fitness_for_max_ret()
        self.var()
#         print("-->after mutation",self.w," and new f = ",self.f)

    def fitness_for_max_ret(self):
        # print("fitting.....")
        self.f = np.array(get_weighted_ret(self.w)).sum()*252
...
#         self.f = np.array(get_weighted_ret(df_data,self.w,symbols_list)).sum()*252
    ...
    def var(self):
        # print("var-ing.....")
        self.v = math.sqrt(portfolio_var(self.w)*252)

    def __init__(self):
        self.w = random.sample(range(0, 100), len(symbols_list))
